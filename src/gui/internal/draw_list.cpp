...
{
	auto range_of_arc(const DrawArcFlag flag) noexcept -> std::pair<int, int>
	{
		...
		switch (flag)
		{
			...
			default: { GAL_PROMETHEUS_ERROR_UNREACHABLE(); } // NOLINT(clang-diagnostic-covered-switch-default)
		}
	}

	class DrawList::Drawer
	{
	public:
		memory::RefWrapper<DrawList> self;

		using path_list_type = std::vector<point_type>;

		path_list_type path_list;

		[[nodiscard]] auto make_appender() noexcept -> PrimitiveAppender
		{
		...
		auto draw_polygon_line(const color_type color, const DrawFlag draw_flag, const float thickness) noexcept -> void
		{
		...
		auto draw_polygon_line_aa(const color_type color, const DrawFlag draw_flag, float thickness) noexcept -> void
		{
			const auto path_point_count = path_list.size();
			const auto& path_point = path_list;
			...
			(
				(draw_list_flag & DrawListFlag::ANTI_ALIASED_LINE_USE_TEXTURE) == DrawListFlag::ANTI_ALIASED_LINE_USE_TEXTURE and
		...
		) noexcept -> void
		{
			...
			appender.add_vertex(rect.right_bottom(), opaque_uv, color_right_bottom);
			appender.add_vertex(rect.left_bottom(), opaque_uv, color_left_bottom);
		...
		) noexcept -> void
		{
			...
			const auto new_texture = draw_list.this_command_texture_id_ != font.texture_id;

			if (new_texture)
			{
				draw_list.push_texture_id(font.texture_id);
			}

			text_draw(font, utf8_text, font_size, wrap_width, p, color, make_appender());
		...
		) noexcept -> void
		{
			...
			const auto new_texture = draw_list.this_command_texture_id_ != texture_id;

			if (new_texture)
			{
				draw_list.push_texture_id(texture_id);
			}
		...
		auto path_stroke(const color_type color, const DrawFlag flag, const float thickness) noexcept -> void
		{
			if (const auto draw_list_flag = self.get().draw_list_flag_;
				(draw_list_flag & DrawListFlag::ANTI_ALIASED_LINE) != DrawListFlag::NONE)
			{
		...
		{
			if (const auto draw_list_flag = self.get().draw_list_flag_;
				(draw_list_flag & DrawListFlag::ANTI_ALIASED_FILL) != DrawListFlag::NONE)
			{
		...
		auto path_arc_fast(const circle_type& circle, const int from, const int to) noexcept -> void
		{
			...
			if (to >= from)
			{
				for (int i = from; i <= to; i += static_cast<int>(step), sample_index += static_cast<int>(step), step = next_step)
				{
					...
					const auto& sample_point = draw_list_shared_data.vertex_sample_point(sample_index);
			...
			if (extra_max_sample)
			{
				...
				const auto& sample_point = draw_list_shared_data.vertex_sample_point(normalized_max_sample_index);

				path_pin({center + sample_point * radius});
			}
		}

		auto path_arc_fast(const circle_type& circle, const DrawArcFlag flag) noexcept -> void
		{
		...
		auto path_arc(const circle_type& circle, const float from, const float to) noexcept -> void
		{
			...
			else
			{
				...
				const auto circle_segment_count = draw_list_shared_data.circle_auto_segment_count(radius);
		...
		auto path_bezier_quadratic_curve_casteljau(const point_type& p1, const point_type& p2, const point_type& p3, const float tessellation_tolerance, const std::size_t level) noexcept -> void
		{
		...
		auto path_bezier_curve(const point_type& p1, const point_type& p2, const point_type& p3, const point_type& p4, const std::uint32_t segments) noexcept -> void
		{
			...
			if (segments == 0)
			{
				GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(draw_list_shared_data.curve_tessellation_tolerance > 0);

				path_reserve_extra(bezier_curve_casteljau_max_level * 2);
				// auto-tessellated
				path_bezier_cubic_curve_casteljau(p1, p2, p3, p4, draw_list_shared_data.curve_tessellation_tolerance, 0);
			}
			else
			{
				path_reserve_extra(segments);
		...
		auto path_bezier_quadratic_curve(const point_type& p1, const point_type& p2, const point_type& p3, const std::uint32_t segments) noexcept -> void
		{
			...
			if (segments == 0)
			{
				...
				path_reserve_extra(bezier_curve_casteljau_max_level * 2);
				// auto-tessellated
				path_bezier_quadratic_curve_casteljau(p1, p2, p3, draw_list_shared_data.curve_tessellation_tolerance, 0);
			}
			else
			{
				path_reserve_extra(segments);
	...
	auto DrawList::on_element_changed(const ChangedElement element) noexcept -> void
	{
		...
		{
			if (
				const auto& [previous_clip_rect, previous_texture, previous_index_offset, previous_element_count] = command_list_[command_count - 2];
	...
	DrawList::DrawList() noexcept
		...
		this_command_texture_id_{Font::invalid_texture_id} {}

	auto DrawList::bind_context(Context& context) noexcept -> void
	{
	...
	auto DrawList::command_list() const noexcept -> const command_list_type&
	{
		return command_list_;
	}

	auto DrawList::vertex_list() const noexcept -> const vertex_list_type&
	{
		return vertex_list_;
	}

	auto DrawList::index_list() const noexcept -> const index_list_type&
	{
		return index_list_;
	}
	...
	) noexcept -> void
	{
		...
		drawer.path_stroke(color, DrawFlag::NONE, thickness);
	}
	...
	) noexcept -> void
	{
		...
		drawer.path_stroke(color, DrawFlag::CLOSED, thickness);
	}
	...
	) noexcept
	{
		...
		if (segments == 0)
		{
			...
			drawer.path_arc_fast(circle, 0, DrawListSharedData::vertex_sample_points_count - 1);
	...
	) noexcept -> void
	{
		...
		{
			// fixme
			segments = draw_list_shared_data.circle_auto_segment_count(std::ranges::max(ellipse.radius.width, ellipse.radius.height));
		}
	...
	) noexcept -> void
	{
		...
		if (wrap_width == text_wrap_width_not_set) // NOLINT(clang-diagnostic-float-equal)
		{
			wrap_width = Font::no_auto_wrap;
		}

		Drawer drawer{.self = *this, .path_list = {}};

		drawer.draw_text(font, font_size, point, color, utf8_text, wrap_width);
	}
