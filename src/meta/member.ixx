...
#endif
{
	...
	template<typename T>
	concept member_gettable_t = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::member_gettable_t<T>;

	template<GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::member_size_gettable_t T>
	[[nodiscard]] constexpr auto member_size() noexcept -> std::size_t //
	{
		return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::member_size<T>();
	}
	...
	{
		return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::visit(
			[]<typename... Ts>(Ts&&... args) noexcept -> decltype(auto) //
			{
				return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::nth_element<N>(std::forward<Ts>(args)...);
			},
			std::forward<T>(value));
	}
	...
	constexpr auto member_walk(Function function, T&& value) noexcept -> void
	{
		...
		{
			(GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::invoke<Index>(function, meta::member_of_index<Index>(std::forward<U>(u))), ...);
	...
	constexpr auto member_walk_until(Function function, T&& value) noexcept -> void
	{
		...
		{
			(GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::invoke<Index>(function, meta::member_of_index<Index>(std::forward<U>(u))) and ...);
		}(std::make_index_sequence<member_size<T>()>{}, std::forward<T>(value));
	}
	...
	) noexcept -> void
	{
		...
		) mutable noexcept -> void //
		{
			...
			{
				GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::invoke<I>(function, meta::member_of_index<I>(std::forward<U>(u))...);
			};
	...
	[[nodiscard]] constexpr auto name_of_member() noexcept -> std::string_view
	{
		constexpr auto full_function_name = get_full_function_name<
			GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::visit(
				[]<typename... Ts>(Ts&&... args) noexcept -> auto //
				{
					return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::wrapper
					{
						GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::nth_element<N>(std::forward<Ts>(args)...)
					};
				},
				GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::extern_any<std::remove_cvref_t<T>>) //
		>();
...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_INTERNAL(meta)
{
	...
	template<typename T, meta::basic_fixed_string Name>
...
#endif
{
	...
	{
		return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_of_member_name<std::remove_cvref_t<T>, Name>() != GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_not_found;
	}
	...
	[[nodiscard]] constexpr auto member_of_name(T&& value) noexcept -> decltype(auto)
	{
		return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::visit(
			[]<typename... Ts>(Ts&&... args) noexcept -> decltype(auto)
			{
				constexpr auto index = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_of_member_name<T, Name>();

				return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::nth_element<index>(std::forward<Ts>(args)...);
			},
			std::forward<T>(value)
		);
	}
