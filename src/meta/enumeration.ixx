...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_INTERNAL(meta)
{
	...
	[[nodiscard]] constexpr auto name_of() noexcept -> std::string_view
	{
		...
		constexpr std::string_view full_function_name = meta::get_full_function_name<DummyEnumDoNotPutIntoAnyNamespace::DO_NOT_USE>();
...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_EXPORT(meta)
{
	...
	namespace user_defined
	{
		...
		struct enum_range
		{
			constexpr static auto min = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::default_min_value<EnumType>();
			constexpr static auto max = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::default_max_value<EnumType>();
		};
...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_INTERNAL(meta)
{
	...
	[[nodiscard]] constexpr auto is_flag_with_user_defined() noexcept -> bool
	{
		...
		else
		{
			...
			return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cached_flag_dynamic_enum_values_size<EnumType>::value >= 6;
		}
	}
...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_EXPORT(meta)
{
	...
	{
		if constexpr (GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::is_flag_with_user_defined<EnumType>())
		{
			return static_cast<std::underlying_type_t<EnumType>>(
				GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::names_of_flag<EnumType, user_defined::enum_name_policy<EnumType>::value>.front().first
			);
		}
		else
		{
			return static_cast<std::underlying_type_t<EnumType>>(
				GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::names_of_enum<EnumType, user_defined::enum_name_policy<EnumType>::value>.front().first
			);
		}
	}
	...
	[[nodiscard]] constexpr auto max_value_of() noexcept -> std::underlying_type_t<EnumType>
	{
		...
		{
			return static_cast<std::underlying_type_t<EnumType>>(
				GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::names_of_flag<EnumType, user_defined::enum_name_policy<EnumType>::value>.back().first
			);
		}
		else
		{
			return static_cast<std::underlying_type_t<EnumType>>(
				GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::names_of_enum<EnumType, user_defined::enum_name_policy<EnumType>::value>.back().first
			);
		}
	}
	...
	[[nodiscard]] constexpr auto names_of() noexcept -> auto
	{
		if constexpr (GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::is_flag_with_user_defined<EnumType>())
		{
			return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::names_of_flag<EnumType, Policy>;
		}
		else
		{
			return GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::names_of_enum<EnumType, Policy>;
		}
	}
	...
	template<EnumNamePolicy Policy, typename Allocator = std::allocator<char>, typename EnumType>
		requires (std::is_enum_v<EnumType> and GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::is_flag_with_user_defined<EnumType>())
