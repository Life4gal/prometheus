...
namespace gal::prometheus::numeric
{
	...
	private:
		...
		GAL_PROMETHEUS_NO_UNIQUE_ADDRESS real_engine_type real_engine_;
	...
	public:
		...
		constexpr static auto seed(const result_type new_seed = static_cast<result_type>(std::chrono::steady_clock::now().time_since_epoch().count())) noexcept(noexcept(engine().seed(new_seed))) -> void//
			requires(is_shared_category) { engine().seed(new_seed); }

		constexpr auto seed(const result_type new_seed = static_cast<result_type>(std::chrono::steady_clock::now().time_since_epoch().count())) noexcept(noexcept(std::declval<Random>().engine().seed(new_seed))) -> void//
			requires(not is_shared_category) { engine().seed(new_seed); }
		...
		constexpr static auto get(
			...
			noexcept(noexcept(std::conditional_t<distribution_compatible_t<integer_distribution_type, T>, integer_distribution_type<T>, floating_point_distribution_type<T>>{from, static_cast<T>(to)}(engine()))) -> T//
			requires(is_shared_category)
		{
			using distribution_type = std::conditional_t<distribution_compatible_t<integer_distribution_type, T>, integer_distribution_type<T>, floating_point_distribution_type<T>>;
		...
		constexpr static auto get(Iterator from, Iterator to) noexcept(noexcept(std::ranges::next(from, get<typename std::iterator_traits<Iterator>::difference_type>(0, 0)))) -> Iterator//
			requires(is_shared_category)
		{
		...
		template<typename Iterator>
			requires requires { typename std::iterator_traits<Iterator>::iterator_category; }
		constexpr auto get(Iterator from, Iterator to) noexcept(noexcept(std::ranges::next(from, get<typename std::iterator_traits<Iterator>::difference_type>(0, 0)))) -> Iterator//
			requires(not is_shared_category)
		{
			if (const auto diff = std::ranges::distance(from, to);
				diff == 0) { return to; }
		...
		template<typename Iterator>
			requires requires { typename std::iterator_traits<Iterator>::iterator_category; }
		constexpr auto operator()(Iterator from, Iterator to) noexcept(noexcept(std::declval<Random>().template get<Iterator>(from, to))) -> Iterator//
		...
		constexpr static auto get(Container&                    container,
		                          const T                       from,
		...
		#if __cpp_static_call_operator >= 202207L
		template<typename Container, typename T>
		constexpr static auto operator()(Container& container, const T from, const std::type_identity_t<T> to, const std::size_t count) noexcept(noexcept(Random::get<Container, T>(container, from, to, count))) -> void//
		 ...
		 */
		template<typename Container, typename T>
		constexpr auto get(Container&                    container,
		                   const T                       from,
		                   const std::type_identity_t<T> to,
		                   const std::size_t             count)
			noexcept(noexcept(
		...
		#if __cpp_static_call_operator >= 202207L
		template<typename Container, typename T>
		constexpr auto operator()(Container& container, const T from, const std::type_identity_t<T> to, const std::size_t count) noexcept(noexcept(std::declval<Random>().template get<Container, T>(container, from, to, count))) -> void//
			requires(not is_shared_category) { return this->template get<Container, T>(container, from, to, count); }
		#endif

		template<typename Container, typename T>
		constexpr static auto get(const T from, const std::type_identity_t<T> to, const std::size_t count) noexcept(noexcept(Random::get(std::declval<Container&>(), from, to, count))) -> Container//
			requires(is_shared_category)
		{
			Container c{};
		...
		#if __cpp_static_call_operator >= 202207L
		template<typename Container, typename T>
		constexpr static auto operator()(const T from, const std::type_identity_t<T> to, const std::size_t count) noexcept(noexcept(Random::get<Container, T>(from, to, count))) -> Container//
			requires(is_shared_category) { return Random::get<Container, T>(from, to, count); }
		#endif

		template<typename Container, typename T>
		constexpr auto get(const T from, const std::type_identity_t<T> to, const std::size_t count) noexcept(noexcept(std::declval<Random>().get(std::declval<Container&>(), from, to, count))) -> Container//
			requires(not is_shared_category)
		{
			Container c{};
		...
		#if __cpp_static_call_operator >= 202207L
		template<typename Container, typename T>
		constexpr auto operator()(const T from, const std::type_identity_t<T> to, const std::size_t count) noexcept(noexcept(std::declval<Random>().template get<Container, T>(from, to, count))) -> Container//
		...
		constexpr static auto get(Args&&... args) noexcept(noexcept(Distribution{std::forward<Args>(args)...}(engine()))) -> typename Distribution::result_type//
			requires(is_shared_category) { return Distribution{std::forward<Args>(args)...}(engine()); }

		#if __cpp_static_call_operator >= 202207L
		template<typename Distribution, typename... Args>
		constexpr static auto operator()(Args&&... args) noexcept(noexcept(Random::get<Distribution, Args...>(std::forward<Args>(args)...))) -> typename Distribution::result_type//
			requires(is_shared_category) { return Random::get<Distribution, Args...>(std::forward<Args>(args)...); }
		#endif

		template<typename Distribution, typename... Args>
		constexpr auto get(Args&&... args) noexcept(noexcept(Distribution{std::forward<Args>(args)...}(std::declval<Random>().engine()))) -> typename Distribution::result_type//
			requires(not is_shared_category) { return Distribution{std::forward<Args>(args)...}(engine()); }

		#if __cpp_static_call_operator >= 202207L
		template<typename Distribution, typename... Args>
		constexpr auto operator()(Args&&... args) noexcept(noexcept(std::declval<Random>().template get<Distribution, Args...>(std::forward<Args>(args)...))) -> typename Distribution::result_type//
