...
{
	auto RectPackContext::nodes_count() const noexcept -> std::size_t
	{
	...
	auto RectPackContext::align_of(const PackPrefer pack_prefer) const noexcept -> std::uint32_t
	{
		...
		{
			return static_cast<std::uint32_t>((static_cast<std::size_t>(size_.width) + nodes_count() - 1) / nodes_count());
		}
	...
	auto RectPackContext::skyline_pack_rectangle(const size_type size, const PackPrefer pack_prefer, const Heuristic heuristic) noexcept -> find_result
	{
		...
		GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(count == nodes_count() + 2);
#endif
	...
	RectPackContext::RectPackContext(const size_type& size) noexcept
		: size_{size},
		  ...
		  active_head_{nodes_.data() + nodes_count()}
	{
		for (auto [index, node]: nodes_ | std::views::take(nodes_count() - 1) | std::views::enumerate)
		{
			node.next = &nodes_[index + 1];
		}
		nodes_[nodes_count() - 1].next = nullptr;
	...
	{
		std::vector<rect_type*> sorted_rects{};
		sorted_rects.reserve(in_out_rects.size());

		std::ranges::transform(
			in_out_rects,
			std::back_inserter(sorted_rects),
			[](rect_type& rect) noexcept -> rect_type*
			{
				return std::addressof(rect);
			}
		);
