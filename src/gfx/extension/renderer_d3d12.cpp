...
#include <gfx/extension/renderer_d3d12.hpp>
...
namespace gal::prometheus::gfx::extension
{
	...
	auto RendererD3D12::create_pipeline_state() noexcept -> bool
	{
	...
	auto RendererD3D12::create_srv_descriptor_heap(const UINT num) noexcept -> bool
	{
	...
	RendererD3D12::RendererD3D12() noexcept
		: device_{nullptr},
		  ...
		  frame_resource_index_{(std::numeric_limits<UINT>::max)()},
		  frame_resource_{} {}

	RendererD3D12::RendererD3D12(ID3D12Device* device, ID3D12GraphicsCommandList* command_list) noexcept
		: RendererD3D12{}
	{
		bind_device(device);
	...
	RendererD3D12::RendererD3D12(ComPtr<ID3D12Device> device, ComPtr<ID3D12GraphicsCommandList> command_list) noexcept
		: RendererD3D12{}
	{
	...
	auto RendererD3D12::bind_device(ID3D12Device* device) noexcept -> void
	{
		GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(device != nullptr);
		device_ = device;
	}

	auto RendererD3D12::bind_device(ComPtr<ID3D12Device> device) noexcept -> void
	{
		GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(device != nullptr);
		device_ = std::move(device);
	}

	auto RendererD3D12::bind_command_list(ID3D12GraphicsCommandList* command_list) noexcept -> void
	{
		GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(command_list != nullptr);
		command_list_ = command_list;
	}

	auto RendererD3D12::bind_command_list(ComPtr<ID3D12GraphicsCommandList> command_list) noexcept -> void
	{
		GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(command_list != nullptr);
	...
	auto RendererD3D12::destruct() noexcept -> void
	{
		...
		std::ranges::for_each(
			...
			[](render_buffer_type& render_buffer) noexcept -> void
			{
				// ComPtr
				render_buffer.vertex = nullptr;
	...
	auto RendererD3D12::present(const render_data_list_type& render_data_list, const extent_type& display_size) noexcept -> void
	{
		...
		// Setup viewport
		{
			const D3D12_VIEWPORT viewport{
					.TopLeftX = .0f,
					.TopLeftY = .0f,
					.Width = static_cast<FLOAT>(display_size.width),
					.Height = static_cast<FLOAT>(display_size.height),
					.MinDepth = .0f,
					.MaxDepth = 1.f
			};
