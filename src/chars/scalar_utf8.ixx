	...
	public:
		...
		[[nodiscard]] constexpr auto rewind_and_validate(const pointer_type begin, const pointer_type current, const pointer_type end) const noexcept -> result_type
		{
			...
			const auto extra_count = std::ranges::distance(
					range |
					std::views::take_while([](const auto byte) noexcept -> bool
		...
		[[nodiscard]] constexpr auto length(const input_type input) const noexcept -> size_type
		{
			...
			else if constexpr (OutputCategory == CharsCategory::UTF32)
			{
				return std::transform_reduce(
						...
						[](const auto byte) noexcept
						{
							return +((byte & 0b1110'0000) == 0b1100'0000);
						});
			}
			else
			{
					...
					if constexpr (CheckNextBlock)
					{
						...
						if (it_input_current + 16 <= it_input_end)
						{
							...
							if (const auto value = [it_input_current]() noexcept
								...
								{
									return utility::unaligned_load<std::uint64_t>(it_input_current + 0);
								}
								else if constexpr (step == sizeof(std::uint64_t) *2)
								{
									const auto v1 = utility::unaligned_load<std::uint64_t>(it_input_current + 0);
									const auto v2 = utility::unaligned_load<std::uint64_t>(it_input_current + sizeof(uint64_t));
									return v1 | v2;
								} }();
								(value & 0x8080'8080'8080'8080) == 0)
							{
