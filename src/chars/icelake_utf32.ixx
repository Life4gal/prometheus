...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_INTERNAL(chars)::icelake_utf32_detail
{
...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_EXPORT(chars)
{
	...
	public:
		...
		[[nodiscard]] constexpr static auto validate(const input_type input) noexcept -> std::conditional_t<ReturnResultType, result_type, bool>
		{
			...
			if constexpr (ReturnResultType)
			{
				...
				if (it_input_current < it_input_end)
				{
					...
					const auto in = _mm512_maskz_loadu_epi32(
			...
			else
			{
				...
				return static_cast<bool>(scalar_type::validate({it_input_current, input_length - (it_input_current - it_input_begin)}));
			}
		}
		...
		[[nodiscard]] constexpr static auto length(const input_type input) noexcept -> size_type
		{
			...
			{
				const auto v_007f = _mm512_set1_epi32(0x0007f);
				...
				for (; it_input_current + 16 <= it_input_end; it_input_current += 16)
				{
					...
					const auto ascii_bitmask = _mm512_cmple_epu32_mask(utf32, v_007f);
					const auto two_bytes_bitmask = _mm512_mask_cmple_epu32_mask(_knot_mask16(ascii_bitmask), utf32, v_07ff);
				...
				return result_length + scalar_type::length<OutputCategory>({it_input_current, input_length - (it_input_current - it_input_begin)});
			}
			...
			{
				const auto v_ffff = _mm512_set1_epi32(0x0000'ffff);
				...
				{
					const auto utf32 = _mm512_loadu_si512(it_input_current);
					const auto surrogates_bitmask = _mm512_cmpgt_epu32_mask(utf32, v_ffff);
		...
		) noexcept -> std::conditional_t<ProcessPolicy == InputProcessPolicy::RETURN_RESULT_TYPE, result_type, std::size_t>
		{
			...
			{
				const auto v_00ff = _mm512_set1_epi32(0x0000'00ff);
				...
				for (; it_input_current + 16 <= it_input_end; it_input_current += 16, it_output_current += 16)
				{
					...
					if (_mm512_cmpgt_epu32_mask(in, v_00ff))
					{
						if constexpr (
			...
			{
				const auto process = [&]() noexcept -> std::conditional_t<ProcessPolicy == InputProcessPolicy::RETURN_RESULT_TYPE, result_type, bool>
				{
					...
					// used iff ProcessPolicy != InputProcessPolicy::RETURN_RESULT_TYPE
					...
					while (it_input_current + 16 + safety_margin <= it_input_end)
					{
						...
						if constexpr (
							ProcessPolicy != InputProcessPolicy::RETURN_RESULT_TYPE or ProcessPolicy ==
							InputProcessPolicy::ASSUME_VALID_INPUT
						...
						if (one_or_two_bytes_bitmask == 0xffff'ffff)
						{
							...
							// 4.pack the bytes
							const auto length_0 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2[static_cast<std::uint8_t>(mask)].front();
							const auto* row_0 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2[static_cast<std::uint8_t>(mask)].data() + 1;
							const auto length_1 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2[static_cast<std::uint8_t>(mask >> 16)].front();
							const auto* row_1 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2[static_cast<std::uint8_t>(mask >> 16)].data() + 1;
						...
						if (saturation_bitmask == 0xffff'ffff)
						{
							...
							const auto length_0 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask)].front();
							const auto* row_0 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask)].data() + 1;
							const auto length_1 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask >> 8)].front();
							const auto* row_1 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask >> 8)].data() + 1;
							const auto length_2 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask >> 16)].front();
							const auto* row_2 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask >> 16)].data() + 1;
							const auto length_3 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask >> 24)].front();
							const auto* row_3 = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::icelake_utf32_detail::utf16_to_utf8::_1_2_3[static_cast<std::uint8_t>(mask >> 24)].data() + 1;
				...
				else if constexpr (ProcessPolicy == InputProcessPolicy::RETURN_RESULT_TYPE) { if (not result) { return result; } }
				else { GAL_PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE(); }

				if (const auto remaining = static_cast<size_type>(it_input_end - it_input_current);
					remaining != 0)
				{
					if (const auto scalar_result = scalar_type::convert<OutputCategory, ProcessPolicy, CheckNextBlock>(
							{it_input_current, remaining},
							it_output_current
						);
						not scalar_result)
