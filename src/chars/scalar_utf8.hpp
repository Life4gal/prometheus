			...
			else
			{
				for (; it_input_current < it_input_end; ++it_input_current)
				{
					if constexpr (CheckNextBlock)
					{
						...
						if (it_input_current + 16 <= it_input_end)
						{
							...
							if (const auto value = [it_input_current]() noexcept
								{
									if constexpr (step == sizeof(std::uint64_t)) { return memory::unaligned_load<std::uint64_t>(it_input_current + 0); }
						...
						leading_byte < 0b1000'0000)
					{
						...
						else if constexpr (OutputCategory == CharsCategory::UTF8) { GAL_PROMETHEUS_UNREACHABLE(); }                                // NOLINT(bugprone-branch-clone)
						else if constexpr (OutputCategory == CharsCategory::UTF16_LE or OutputCategory == CharsCategory::UTF16_BE)
						{
							*it_output_current = [leading_byte]() noexcept -> auto
							{
								if constexpr ((OutputCategory == CharsCategory::UTF16_LE) != (std::endian::native == std::endian::little)) { return std::byteswap(static_cast<output_char_type>(leading_byte)); }
								else { return static_cast<output_char_type>(leading_byte); }
							}();
						}
						else if constexpr (OutputCategory == CharsCategory::UTF32) { *it_output_current = static_cast<output_char_type>(leading_byte); }
					...
					else if ((leading_byte & 0b1110'0000) == 0b1100'0000)
					{
						...
						if (it_input_current + 1 >= it_input_end)
						{
							...
							else if constexpr (Criterion == InputProcessCriterion::RETURN_RESULT_TYPE) { return result_type{.error = ErrorCode::TOO_SHORT, .count = length_if_error}; }
						...
						if constexpr (Criterion != InputProcessCriterion::ASSUME_VALID_INPUT)
						{
							...
							{
								if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT) { return 0; }
								else if constexpr (Criterion == InputProcessCriterion::RETURN_RESULT_TYPE) { return result_type{.error = ErrorCode::OVERLONG, .count = length_if_error}; }
								else { GAL_PROMETHEUS_STATIC_UNREACHABLE(); }
							}
							if (code_point >
							    []() noexcept -> auto
						...
						*it_output_current = static_cast<output_char_type>(
							[code_point]() noexcept
							{
								if constexpr ((OutputCategory == CharsCategory::UTF16_LE) != (std::endian::native == std::endian::little)) { return std::byteswap(code_point); }
					...
					else if ((leading_byte & 0b1111'0000) == 0b1110'0000)
					{
						...
						else if constexpr (OutputCategory == CharsCategory::UTF8) { GAL_PROMETHEUS_STATIC_UNREACHABLE(); }
						else if constexpr (OutputCategory == CharsCategory::UTF16_LE or OutputCategory == CharsCategory::UTF16_BE or OutputCategory == CharsCategory::UTF32)
						{
							...
							if constexpr (Criterion != InputProcessCriterion::ASSUME_VALID_INPUT)
							{
								...
								{
									if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT) { return 0; }
									if constexpr (Criterion == InputProcessCriterion::RETURN_RESULT_TYPE) { return result_type{.error = ErrorCode::TOO_SHORT, .count = length_if_error}; }
									else { GAL_PROMETHEUS_STATIC_UNREACHABLE(); }
								}
							}
							...
							if constexpr (Criterion != InputProcessCriterion::ASSUME_VALID_INPUT)
							{
								...
								{
									if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT) { return 0; }
									else if constexpr (Criterion == InputProcessCriterion::RETURN_RESULT_TYPE) { return result_type{.error = error, .count = length_if_error}; }
									else { GAL_PROMETHEUS_STATIC_UNREACHABLE(); }
								};
							...
							*it_output_current = [cp = static_cast<output_char_type>(code_point)]() noexcept
							{
								if constexpr (OutputCategory != CharsCategory::UTF32 and (OutputCategory == CharsCategory::UTF16_LE) != (std::endian::native == std::endian::little)) { return std::byteswap(cp); }
								else { return cp; }
							}();
		...
		[[nodiscard]] constexpr auto rewind_and_convert(const pointer_type furthest_possible_begin, const input_type input, typename output_type<OutputCategory>::pointer output) const noexcept -> result_type
		{
			...
			const auto extra_count = std::ranges::distance(
					...
					std::views::take_while(
							...
							{
								if constexpr (OutputCategory == CharsCategory::ASCII) { return (c & 0b1100'0000) != 0b1000'0000; }// NOLINT(bugprone-branch-clone)
								else if constexpr (OutputCategory == CharsCategory::UTF16_LE or OutputCategory == CharsCategory::UTF16_BE) { return (c & 0b1100'0000) != 0b1000'0000; }
								else { GAL_PROMETHEUS_STATIC_UNREACHABLE(); }
							}));
