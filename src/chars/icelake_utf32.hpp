...
namespace gal::prometheus::chars
{
	...
	public:
		...
		[[nodiscard]] constexpr auto validate(const input_type input) const noexcept -> std::conditional_t<ReturnResultType, result_type, bool>
		{
			...
			if constexpr (ReturnResultType)
			{
				for (; it_input_current + 16 <= it_input_end; it_input_current += 16)
				{
					...
					if (const auto surrogate_range = _mm512_cmp_epu32_mask(utf32_offset, _mm512_set1_epi32(static_cast<int>(0xffff'f7ff)), _MM_CMPINT_GT);
				...
				if (it_input_current < it_input_end)
				{
					...
					const auto utf32 = _mm512_maskz_loadu_epi32(static_cast<__mmask16>((__mmask16{1} << (it_input_end - it_input_current)) - 1), it_input_current);
					if (const auto outside_range = _mm512_cmp_epu32_mask(utf32, _mm512_set1_epi32(0x10'ffff), _MM_CMPINT_GT);
						outside_range) { return result_type{.error = ErrorCode::TOO_LARGE, .count = length_if_error + std::countr_zero(outside_range)}; }
		...
		[[nodiscard]] constexpr auto length(const input_type input) const noexcept -> size_type
		{
			...
			if constexpr (OutputCategory == CharsCategory::ASCII) { return instance::scalar_utf32.length<OutputCategory>(input); }// NOLINT(bugprone-branch-clone)
			...
			else if constexpr (OutputCategory == CharsCategory::UTF8)
			{
				const auto process = [&]() noexcept -> std::conditional_t<Criterion == InputProcessCriterion::RETURN_RESULT_TYPE, result_type, bool>
				{
					...
					while (it_input_current + 16 + safety_margin <= it_input_end)
					{
						...
						if constexpr (Criterion != InputProcessCriterion::RETURN_RESULT_TYPE or Criterion == InputProcessCriterion::ASSUME_VALID_INPUT) { running_max = _mm256_max_epu32(_mm256_max_epu32(in, running_max), next_in); }
						else
						{
							// Check for too large input
							if (const auto max_input = _mm256_max_epu32(_mm256_max_epu32(in, next_in), v_0010_ffff);
								static_cast<std::uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi32(max_input, v_0010_ffff))) != 0xffff'ffff) { return result_type{.error = ErrorCode::TOO_LARGE, .count = length_if_error}; }
						}
						...
						const auto saturation_byte_mask = _mm256_cmpeq_epi32(_mm256_and_si256(_mm256_or_si256(in, next_in), v_ffff_0000), v_0000_0000);
						...
						if (saturation_bitmask == 0xffff'ffff)
						{
							...
							if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT or Criterion == InputProcessCriterion::ASSUME_VALID_INPUT) { forbidden_byte_mask = _mm256_or_si256(forbidden_byte_mask, _mm256_cmpeq_epi16(_mm256_and_si256(in_16, v_0000_f800), v_0000_d800)); }
							else if constexpr (Criterion == InputProcessCriterion::RETURN_RESULT_TYPE)
							{
								forbidden_byte_mask = _mm256_cmpeq_epi16(_mm256_and_si256(in_16, v_0000_f800), v_0000_d800);
								if (static_cast<std::uint32_t>(_mm256_movemask_epi8(forbidden_byte_mask)) != 0x0) { return result_type{.error = ErrorCode::SURROGATE, .count = length_if_error}; }
							}
							...
							const auto s4 = _mm256_xor_si256(s3, _mm256_andnot_si256(one_or_two_bytes_byte_mask, _mm256_set1_epi16(0b0100'0000'0000'0000)));
						...
						else
						{
							...
							if constexpr (auto result = instance::scalar_utf32.convert<OutputCategory, Criterion, CheckNextBlock>({it_input_current, step}, it_output_current);
					...
					if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT or Criterion == InputProcessCriterion::ASSUME_VALID_INPUT)
					{
						...
						if (static_cast<std::uint32_t>(_mm256_movemask_epi8(_mm256_cmpeq_epi32(_mm256_max_epu32(running_max, v_0010_ffff), v_0010_ffff))) != 0xffff'ffff) { return false; }
				...
				if (const auto remaining = static_cast<size_type>(it_input_end - it_input_current);
					remaining != 0)
				{
					if (const auto scalar_result = instance::scalar_utf32.convert<OutputCategory, Criterion, CheckNextBlock>({it_input_current, remaining}, it_output_current);
						not scalar_result)
					{
						if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT or Criterion == InputProcessCriterion::ASSUME_VALID_INPUT) { return 0; }
			...
			else if constexpr (OutputCategory == CharsCategory::UTF16_LE or OutputCategory == CharsCategory::UTF16_BE)
			{
				const auto process = [&]() noexcept -> std::conditional_t<Criterion == InputProcessCriterion::RETURN_RESULT_TYPE, result_type, bool>
				{
					...
					while (it_input_current + 8 + safety_margin <= it_input_end)
					{
						...
						if (saturation_bitmask == 0xffff'ffff)
						{
							if constexpr (Criterion == InputProcessCriterion::ZERO_IF_ERROR_ELSE_PROCESSED_OUTPUT or Criterion == InputProcessCriterion::ASSUME_VALID_INPUT) { forbidden_byte_mask = _mm256_or_si256(forbidden_byte_mask, _mm256_cmpeq_epi32(_mm256_and_si256(in, v_0000_f800), v_0000_d800)); }
							else if constexpr (Criterion == InputProcessCriterion::RETURN_RESULT_TYPE)
							{
							...
							{
								if constexpr ((OutputCategory == CharsCategory::UTF16_LE) != (std::endian::native == std::endian::little)) { return _mm_shuffle_epi8(packed, _mm_setr_epi8(1, 0, 3, 2, 5, 4, 7, 6, 9, 8, 11, 10, 13, 12, 15, 14)); }
								else { return packed; }
							}();
