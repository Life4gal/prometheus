...
namespace
{
	...
	namespace latin
	{
		using input_type = scalar::latin::input_type;
		using char_type = scalar::latin::char_type;
		using size_type = scalar::latin::size_type;
		using pointer_type = scalar::latin::pointer_type;
		...
		[[nodiscard]] constexpr auto validate_one(const pointer_type current, const pointer_type end) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			std::ignore = end;
		...
		[[nodiscard]] constexpr auto validate_range(const input_type input) noexcept -> result_error_input_type
		{
			...
			if (remaining != 0)
			{
				...
				while (it_input_current < end)
				{
					...
					const auto [length, error] = ::latin::validate_one(it_input_current, it_input_end);
		...
		[[nodiscard]] constexpr auto write_one_utf8(
			typename output_type_of<OutputType>::pointer& output,
		...
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			...
			if constexpr (const auto value = static_cast<std::uint8_t>(*(current + 0));
				Pure)
			{
				*(output + 0) = static_cast<output_char_type>(value);
			...
			else
			{
				...
				*(output + 0) = static_cast<output_char_type>(c1);
				*(output + 1) = static_cast<output_char_type>(c2);
		...
		[[nodiscard]] constexpr auto write_range_utf8(
			typename output_type_of<OutputType>::pointer& output,
			const input_type input
		) noexcept -> result_error_input_output_type
		{
			...
			const auto transform = [&]<bool P>(const decltype(advance) n) noexcept -> void
			{
				...
				while (it_input_current < end)
				{
					const auto [length, error] = latin::write_one_utf8<OutputType, P, Correct>(it_output_current, it_input_current, it_input_end);
		...
		[[nodiscard]] constexpr auto write_one_utf16(
			typename output_type_of<OutputType>::pointer& output,
			const pointer_type current,
			const pointer_type end
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			...
			*(output + 0) = static_cast<output_char_type>(common::to_native_utf16<OutputType>(static_cast<std::uint16_t>(value)));
		...
		[[nodiscard]] constexpr auto write_range_utf16(
			typename output_type_of<OutputType>::pointer& output,
			const input_type input
		) noexcept -> result_error_input_output_type
		{
			...
			const auto transform = [&]<bool P>(const decltype(advance) n) noexcept -> void
			{
				...
				while (it_input_current < end)
				{
					const auto [length, error] = latin::write_one_utf16<OutputType, P, Correct>(it_output_current, it_input_current, it_input_end);
					GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(length == 1);
					GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(error == ErrorCode::NONE);
		...
		[[nodiscard]] constexpr auto write_one_utf32(
			typename output_type_of<OutputType>::pointer& output,
			const pointer_type current,
			const pointer_type end
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		...
		[[nodiscard]] constexpr auto write_range_utf32(
			typename output_type_of<OutputType>::pointer& output,
			const input_type input
		) noexcept -> result_error_input_output_type
		{
			...
			const auto transform = [&]<bool P>(const decltype(advance) n) noexcept -> void
			{
				...
				while (it_input_current < end)
				{
					const auto [length, error] = latin::write_one_utf32<OutputType, P, Correct>(it_output_current, it_input_current, it_input_end);
					GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(length == 1);
					GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(error == ErrorCode::NONE);
	...
	namespace utf8
	{
		...
		[[nodiscard]] constexpr auto validate_one(
		...
		[[nodiscard]] constexpr auto validate_range(const input_type_of<InputType> input) noexcept -> result_error_input_type
		{
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(input.data() != nullptr);
			...
			constexpr auto advance = utf8::advance_of<InputType, InputType>();
			...
			const auto check = [&](const size_type n) noexcept -> result_error_input_type
			{
				...
				while (it_input_current < end)
				{
					...
					const auto [length, error] = utf8::validate_one<InputType>(it_input_current, it_input_end);
					if (error != ErrorCode::NONE)
					{
						return {.error = error, .input = current_input_length};
					}
			...
			while (it_input_current + advance <= it_input_end)
			{
				...
				const auto data = utf8::read<InputType, InputType>(it_input_current);
		...
		[[nodiscard]] constexpr auto write_one_latin(
			typename output_type_of<OutputType>::pointer& output,
			const typename input_type_of<InputType>::const_pointer current,
			const typename input_type_of<InputType>::const_pointer end
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		...
		[[nodiscard]] constexpr auto write_range_latin(
			typename output_type_of<OutputType>::pointer& output,
			const input_type_of<InputType> input
		) noexcept -> result_error_input_output_type
		{
			...
			const auto advance = utf8::advance_of<InputType, OutputType>();
			const auto transform = [&]<bool P>(const decltype(advance) n) noexcept -> result_error_input_output_type
			{
				...
				while (it_input_current < end)
				{
					...
					const auto [length, error] = utf8::write_one_latin<InputType, OutputType, P, Correct>(it_output_current, it_input_current, it_input_end);
					if (error != ErrorCode::NONE)
					{
			...
			while (it_input_current + advance <= it_input_end)
			{
				...
				const auto data = utf8::read<InputType, OutputType>(it_input_current);
		...
		) noexcept -> result_error_input_output_type
		{
			...
			const auto transform = [&]<bool P>(const decltype(advance) n) noexcept -> result_error_input_output_type
			{
				...
				while (it_input_current < end)
				{
					...
					const auto [length, error] = utf8::write_one_utf16<InputType, OutputType, P, Correct>(it_output_current, it_input_current, it_input_end);
					if (error != ErrorCode::NONE)
					{
						return {.error = error, .input = current_input_length, .output = current_output_length};
					}
		...
		) noexcept -> result_error_input_output_type
		{
			...
			const auto transform = [&]<bool P>(const decltype(advance) n) noexcept -> result_error_input_output_type
			{
				...
				while (it_input_current < end)
				{
					...
					const auto [length, error] = utf8::write_one_utf32<InputType, OutputType, P, Correct>(it_output_current, it_input_current, it_input_end);
					if (error != ErrorCode::NONE)
					{
						return {.error = error, .input = current_input_length, .output = current_output_length};
					}
		...
		[[nodiscard]] constexpr auto transform_one(
			typename output_type_of<OutputType>::pointer& output,
			const typename input_type_of<InputType>::const_pointer current,
			const typename input_type_of<InputType>::const_pointer end
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		...
		[[nodiscard]] constexpr auto transform_range(
			typename output_type_of<OutputType>::pointer& output,
			const input_type_of<InputType> input
		) noexcept -> result_error_input_output_type
		{
			...
			{
				if (const auto result = utf8::validate_range<InputType>(input);
					result.has_error())
				{
...
namespace gal::prometheus::chars_1::scalar
{
	namespace latin
	{
		...
		{
			return ::latin::validate_one(current, end);
		}

		auto validate(const input_type input) noexcept -> result_error_input_type
		{
			return ::latin::validate_range(input);
		}
		...
		{
			const auto length = ::latin::length<CharsType::UTF8_CHAR>(input);
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(length == ::latin::length<CharsType::UTF8>(input));
		...
		{
			const auto length = ::latin::length<CharsType::UTF16>(input);
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(length == ::latin::length<CharsType::UTF16_LE>(input));
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME(length == ::latin::length<CharsType::UTF16_BE>(input));
		...
		{
			return ::latin::length<CharsType::UTF32>(input);
		}
		...
		{
			return ::latin::write_one_utf8<CharsType::UTF8_CHAR, false, false>(output, current, end);
		}
		...
		{
			return ::latin::write_range_utf8<CharsType::UTF8_CHAR, false, false>(output, input);
		}
		...
		{
			return ::latin::write_one_utf8<CharsType::UTF8_CHAR, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf8<CharsType::UTF8_CHAR, true, false>(output, input);
		...
		{
			return ::latin::write_one_utf8<CharsType::UTF8_CHAR, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf8<CharsType::UTF8_CHAR, false, true>(output, input);
		...
		{
			return ::latin::write_one_utf8<CharsType::UTF8, false, false>(output, current, end);
		}
		...
		{
			return ::latin::write_range_utf8<CharsType::UTF8, false, false>(output, input);
		}
		...
		{
			return ::latin::write_one_utf8<CharsType::UTF8, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf8<CharsType::UTF8, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return ::latin::write_one_utf8<CharsType::UTF8, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf8<CharsType::UTF8, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return ::latin::write_one_utf16<CharsType::UTF16_LE, false, false>(output, current, end);
		}
		...
		{
			return ::latin::write_range_utf16<CharsType::UTF16_LE, false, false>(output, input);
		}
		...
		{
			return ::latin::write_one_utf16<CharsType::UTF16_LE, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf16<CharsType::UTF16_LE, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return ::latin::write_one_utf16<CharsType::UTF16_LE, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf16<CharsType::UTF16_LE, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return ::latin::write_range_utf16<CharsType::UTF16_BE, false, false>(output, input);
		}
		...
		{
			const auto result = ::latin::write_range_utf16<CharsType::UTF16_BE, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return ::latin::write_one_utf16<CharsType::UTF16_BE, false, false>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf16<CharsType::UTF16_BE, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return ::latin::write_one_utf32<CharsType::UTF32, false, false>(output, current, end);
		}
		...
		{
			return ::latin::write_range_utf32<CharsType::UTF32, false, false>(output, input);
		}
		...
		{
			return ::latin::write_one_utf32<CharsType::UTF32, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::latin::write_range_utf32<CharsType::UTF32, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			const auto result = ::latin::write_range_utf32<CharsType::UTF32, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return ::latin::write_latin<CharsType::LATIN, false, false>(output, input);
		}
		...
		{
			const auto result = write_latin(output, input);
			return {.error = result.error, .input = result.input};
		}
	...
	namespace utf8
	{
		[[nodiscard]] auto validate(const uc::pointer_type current, const uc::pointer_type end) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			return ::utf8::validate_one<CharsType::UTF8_CHAR>(current, end);
		}

		auto validate(const uc::input_type input) noexcept -> result_error_input_type
		{
			return ::utf8::validate_range<CharsType::UTF8_CHAR>(input);
		}

		auto validate(const uc::pointer_type input) noexcept -> result_error_input_type
		{
			return validate({input, std::char_traits<uc::char_type>::length(input)});
		}

		auto length_for_latin(const uc::input_type input) noexcept -> uc::size_type
		{
			return ::utf8::length<CharsType::UTF8_CHAR, CharsType::LATIN>(input);
		}

		auto length_for_latin(const uc::pointer_type input) noexcept -> uc::size_type
		{
			return length_for_latin({input, std::char_traits<uc::char_type>::length(input)});
		}

		auto length_for_utf16(const uc::input_type input) noexcept -> uc::size_type
		{
			const auto length = ::utf8::length<CharsType::UTF8_CHAR, CharsType::UTF16>(input);
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME((length == ::utf8::length<CharsType::UTF8_CHAR, CharsType::UTF16_LE>(input)));
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME((length == ::utf8::length<CharsType::UTF8_CHAR, CharsType::UTF16_BE>(input)));

			return length;
		}

		auto length_for_utf16(const uc::pointer_type input) noexcept -> uc::size_type
		{
			return length_for_utf16({input, std::char_traits<uc::char_type>::length(input)});
		}

		auto length_for_utf32(const uc::input_type input) noexcept -> uc::size_type
		{
			return ::utf8::length<CharsType::UTF8_CHAR, CharsType::UTF32>(input);
		}

		auto length_for_utf32(uc::pointer_type input) noexcept -> uc::size_type
		{
			return length_for_utf32({input, std::char_traits<uc::char_type>::length(input)});
		}

		[[nodiscard]] auto write_latin(
			output_type_of<CharsType::LATIN>::pointer& output,
			const uc::pointer_type current,
			const uc::pointer_type end
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			return ::utf8::write_one_latin<CharsType::UTF8_CHAR, CharsType::LATIN, false, false>(output, current, end);
		}

		auto write_latin(
			output_type_of<CharsType::LATIN>::pointer& output,
			const uc::input_type input
		) noexcept -> result_error_input_output_type
		{
			return ::utf8::write_range_latin<CharsType::UTF8_CHAR, CharsType::LATIN, false, false>(output, input);
		}

		auto write_latin(
			output_type_of<CharsType::LATIN>::pointer& output,
			const uc::pointer_type input
		) noexcept -> result_error_input_output_type
		{
			return write_latin(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_latin<CharsType::UTF8_CHAR, CharsType::LATIN, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_latin<CharsType::UTF8_CHAR, CharsType::LATIN, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_latin_pure(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_latin<CharsType::UTF8_CHAR, CharsType::LATIN, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_latin<CharsType::UTF8_CHAR, CharsType::LATIN, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_latin_correct(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_LE, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::write_range_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_LE, false, false>(output, input);
		}
		...
		{
			return write_utf16_le(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_LE, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_LE, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf16_le_pure(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_LE, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_LE, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf16_le_correct(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_BE, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::write_range_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_BE, false, false>(output, input);
		}
		...
		{
			return write_utf16_be(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_BE, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_BE, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf16_be_pure(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_BE, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8_CHAR, CharsType::UTF16_BE, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf16_be_correct(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf32<CharsType::UTF8_CHAR, CharsType::UTF32, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::write_range_utf32<CharsType::UTF8_CHAR, CharsType::UTF32, false, false>(output, input);
		}
		...
		{
			return write_utf32(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf32<CharsType::UTF8_CHAR, CharsType::UTF32, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf32<CharsType::UTF8_CHAR, CharsType::UTF32, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf32_pure(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf32<CharsType::UTF8_CHAR, CharsType::UTF32, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf32<CharsType::UTF8_CHAR, CharsType::UTF32, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf32_correct(output, {input, std::char_traits<uc::char_type>::length(input)});
		}

		[[nodiscard]] auto validate(const u::pointer_type current, const u::pointer_type end) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			return ::utf8::validate_one<CharsType::UTF8>(current, end);
		}

		[[nodiscard]] auto validate(const u::input_type input) noexcept -> result_error_input_type
		{
			return ::utf8::validate_range<CharsType::UTF8>(input);
		}

		[[nodiscard]] auto validate(const u::pointer_type input) noexcept -> result_error_input_type
		{
			return validate({input, std::char_traits<u::char_type>::length(input)});
		}

		auto length_for_latin(const u::input_type input) noexcept -> uc::size_type
		{
			return ::utf8::length<CharsType::UTF8, CharsType::LATIN>(input);
		}

		auto length_for_latin(const u::pointer_type input) noexcept -> uc::size_type
		{
			return length_for_latin({input, std::char_traits<u::char_type>::length(input)});
		}

		auto length_for_utf16(const u::input_type input) noexcept -> uc::size_type
		{
			const auto length = ::utf8::length<CharsType::UTF8, CharsType::UTF16>(input);
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME((length == ::utf8::length<CharsType::UTF8, CharsType::UTF16_LE>(input)));
			GAL_PROMETHEUS_ERROR_DEBUG_ASSUME((length == ::utf8::length<CharsType::UTF8, CharsType::UTF16_BE>(input)));

			return length;
		}

		auto length_for_utf16(const u::pointer_type input) noexcept -> uc::size_type
		{
			return length_for_utf16({input, std::char_traits<u::char_type>::length(input)});
		}

		auto length_for_utf32(const u::input_type input) noexcept -> uc::size_type
		{
			return ::utf8::length<CharsType::UTF8, CharsType::UTF32>(input);
		}

		auto length_for_utf32(const u::pointer_type input) noexcept -> uc::size_type
		{
			return length_for_utf32({input, std::char_traits<u::char_type>::length(input)});
		}

		[[nodiscard]] auto write_latin(
			output_type_of<CharsType::LATIN>::pointer& output,
			const u::pointer_type current,
			const u::pointer_type end
		) noexcept -> std::pair<std::ptrdiff_t, ErrorCode>
		{
			return ::utf8::write_one_latin<CharsType::UTF8, CharsType::LATIN, false, false>(output, current, end);
		}

		auto write_latin(
			output_type_of<CharsType::LATIN>::pointer& output,
			const u::input_type input
		) noexcept -> result_error_input_output_type
		{
			return ::utf8::write_range_latin<CharsType::UTF8, CharsType::LATIN, false, false>(output, input);
		}

		auto write_latin(
			output_type_of<CharsType::LATIN>::pointer& output,
			const u::pointer_type input
		) noexcept -> result_error_input_output_type
		{
			return write_latin(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_latin<CharsType::UTF8, CharsType::LATIN, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_latin<CharsType::UTF8, CharsType::LATIN, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_latin_pure(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_latin<CharsType::UTF8, CharsType::LATIN, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_latin<CharsType::UTF8, CharsType::LATIN, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_latin_correct(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8, CharsType::UTF16_LE, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::write_range_utf16<CharsType::UTF8, CharsType::UTF16_LE, false, false>(output, input);
		}
		...
		{
			return write_utf16_le(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8, CharsType::UTF16_LE, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8, CharsType::UTF16_LE, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf16_le_pure(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8, CharsType::UTF16_LE, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8, CharsType::UTF16_LE, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf16_le_correct(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8, CharsType::UTF16_BE, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::write_range_utf16<CharsType::UTF8, CharsType::UTF16_BE, false, false>(output, input);
		}
		...
		{
			return write_utf16_be(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8, CharsType::UTF16_BE, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8, CharsType::UTF16_BE, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf16_be_pure(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf16<CharsType::UTF8, CharsType::UTF16_BE, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf16<CharsType::UTF8, CharsType::UTF16_BE, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf16_be_correct(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf32<CharsType::UTF8, CharsType::UTF32, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::write_range_utf32<CharsType::UTF8, CharsType::UTF32, false, false>(output, input);
		}
		...
		{
			return write_utf32(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf32<CharsType::UTF8, CharsType::UTF32, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf32<CharsType::UTF8, CharsType::UTF32, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf32_pure(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::write_one_utf32<CharsType::UTF8, CharsType::UTF32, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::write_range_utf32<CharsType::UTF8, CharsType::UTF32, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf32_correct(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::transform_one<CharsType::UTF8_CHAR, CharsType::UTF8, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::transform_range<CharsType::UTF8_CHAR, CharsType::UTF8, false, false>(output, input);
		}
		...
		{
			return write_utf8(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::transform_one<CharsType::UTF8_CHAR, CharsType::UTF8, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::transform_range<CharsType::UTF8_CHAR, CharsType::UTF8, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf8_pure(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			return ::utf8::transform_one<CharsType::UTF8_CHAR, CharsType::UTF8, false, true>(output, current, end);
		}
		...
		{
			return write_utf8_correct(output, {input, std::char_traits<uc::char_type>::length(input)});
		}
		...
		{
			const auto result = ::utf8::transform_range<CharsType::UTF8_CHAR, CharsType::UTF8, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return ::utf8::transform_one<CharsType::UTF8, CharsType::UTF8_CHAR, false, false>(output, current, end);
		}
		...
		{
			return ::utf8::transform_range<CharsType::UTF8, CharsType::UTF8_CHAR, false, false>(output, input);
		}
		...
		{
			return write_utf8(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::transform_one<CharsType::UTF8, CharsType::UTF8_CHAR, true, false>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::transform_range<CharsType::UTF8, CharsType::UTF8_CHAR, true, false>(output, input);
			return {.error = result.error, .input = result.input};
		}
		...
		{
			return write_utf8_pure(output, {input, std::char_traits<u::char_type>::length(input)});
		}
		...
		{
			return ::utf8::transform_one<CharsType::UTF8, CharsType::UTF8_CHAR, false, true>(output, current, end);
		}
		...
		{
			const auto result = ::utf8::transform_range<CharsType::UTF8, CharsType::UTF8_CHAR, false, true>(output, input);
			return {.output = result.output};
		}
		...
		{
			return write_utf8_correct(output, {input, std::char_traits<u::char_type>::length(input)});
		}
	}
