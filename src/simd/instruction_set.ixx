...
export module gal.prometheus.simd:instruction_set;

import std;
import gal.prometheus.error;
import gal.prometheus.utility;

export namespace gal::prometheus::simd
{
	...
	namespace instruction_set_detail
	{
	...
	auto detect_supported_architectures() -> std::uint32_t
	{
		std::uint32_t host_isa = 0;

		// EAX = 0x01H
		{
			constexpr instruction_set_detail::cpu_id_t config{.eax = 0x01, .ebx = 0, .ecx = 0, .edx = 0};

			const auto [eax, ebx, ecx, edx] = get_cpu_id(config);

			if (ecx & instruction_set_detail::pclmulqdq) { host_isa |= static_cast<std::uint32_t>(InstructionSet::PCLMULQDQ); }
			if (ecx & instruction_set_detail::sse42) { host_isa |= static_cast<std::uint32_t>(InstructionSet::SSE42); }
			if ((ecx & instruction_set_detail::osxsave) != instruction_set_detail::osxsave) { return host_isa; }
		}

		// checking if the OS saves registers
		const auto xcr0 = instruction_set_detail::get_xcr0();
		if ((xcr0 & instruction_set_detail::xcr0::avx256saved) == 0) { return host_isa; }

		// EAX = 0x7H
		{
			constexpr instruction_set_detail::cpu_id_t config{.eax = 0x07, .ebx = 0, .ecx = 0, .edx = 0};

			const auto [eax, ebx, ecx, edx] = get_cpu_id(config);

			if (ebx & instruction_set_detail::ebx::bmi1) { host_isa |= static_cast<std::uint32_t>(InstructionSet::BMI1); }
			if (ebx & instruction_set_detail::ebx::avx2) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX2); }
			if (ebx & instruction_set_detail::ebx::bmi2) { host_isa |= static_cast<std::uint32_t>(InstructionSet::BMI2); }

			if ((xcr0 & instruction_set_detail::xcr0::avx512saved) != instruction_set_detail::xcr0::avx512saved) { return host_isa; }

			if (ebx & instruction_set_detail::ebx::avx512f) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512F); }
			if (ebx & instruction_set_detail::ebx::avx512dq) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512DQ); }
			if (ebx & instruction_set_detail::ebx::avx512cd) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512CD); }
			if (ebx & instruction_set_detail::ebx::avx512bw) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512BW); }
			if (ebx & instruction_set_detail::ebx::avx512vl) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512VL); }

			if (ecx & instruction_set_detail::ecx::avx512vbmi2) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512VBMI2); }
			if (ecx & instruction_set_detail::ecx::avx512vpopcntdq) { host_isa |= static_cast<std::uint32_t>(InstructionSet::AVX512VPOPCNTDQ); }
		}
	...
	namespace instruction_set_detail
	{
		constexpr utility::Enumerations instruction_set_meta{
				...
				"AVX512F",
				InstructionSet::AVX512DQ,
				"AVX512DQ",
				InstructionSet::AVX512CD,
				"AVX512CD",
				InstructionSet::AVX512BW,
				"AVX512BW",
				InstructionSet::AVX512VL,
				"AVX512VL",
				InstructionSet::AVX512VBMI2,
				"AVX512VBMI2",
	...
	[[nodiscard]] constexpr auto to_string(const InstructionSet instruction) -> decltype(auto) { return instruction_set_detail::instruction_set_meta[instruction]; }

	// ReSharper restore StringLiteralTypo
	// ReSharper restore CommentTypo
	// ReSharper restore IdentifierTypo
	// ReSharper restore CppInconsistentNaming
}// namespace gal::prometheus::simd
