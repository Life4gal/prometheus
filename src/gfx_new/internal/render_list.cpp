...
namespace gal::prometheus::gfx_new
{
	...
	public:
		...
		mutable memory::RefWrapper<Renderer> renderer;
		...
		{
			const Renderer::AccessorRender accessor{renderer.get()};

			return accessor.context().render_list_shared_data;
		}

		[[nodiscard]] auto default_texture() const noexcept -> texture_id_type
		{
			const Renderer::AccessorTexture accessor{renderer.get()};

			return accessor.context().root().id();
		}
		...
		auto push_scissor(const rect_type& rect, const bool intersect_with_current_scissor) noexcept -> rect_type&
		{
	...
	RenderList::RenderList(Renderer& renderer) noexcept
		: context_{memory::make_unique<RenderListContext>(renderer, RenderListFlag::DEFAULT)} {}
	...
	) noexcept -> void
	{
		...
		RenderListDrawer drawer{.self = *context_, .path_list = {}};
	...
	) noexcept -> void
	{
		...
		{
			// fixme
			segments = context_->shared_data().circle_auto_segment_count(std::ranges::max(ellipse.radius.width, ellipse.radius.height));
		}
