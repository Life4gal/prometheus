...
import :element.flex; // for detail::element::make_box_filler
...
#include <draw/detail/element.flex.ixx> // for detail::element::make_box_filler
...
namespace gal::prometheus::draw
{
	...
	namespace element
	{
		constexpr auto box = detail::element::box_options{};
		constexpr auto center = detail::element::center_options{};
	}
	...
	namespace detail
	{
		namespace element
		{
			...
			[[nodiscard]] inline auto make_box_filler() noexcept -> element_type
			{
				return make(draw::element::flex_auto)();
			}
			...
			private:
				...
				constexpr static auto fill_line(elements_type& dest, const size_type width, derived_elements_t auto&& elements) noexcept -> void
				{
					...
					if (const auto diff = width - size;
						diff != 0)
					{
						const auto filler = make_box_filler();
		...
		template<element::box_option_t auto... Os>
		struct maker<Os...>
		{
			template<element::BoxOption Option>
				requires (Option == element::BoxOption::HORIZONTAL or Option == element::BoxOption::VERTICAL)
			[[nodiscard]] auto operator()(options<Option>) const noexcept -> element::box_builder<Option>
			{
				return {};
			}

			template<element::BoxOption Option, typename... Args>
				requires (Option == element::BoxOption::HORIZONTAL or Option == element::BoxOption::VERTICAL)
			...
			template<element::BoxOption Option>
				requires (Option == element::BoxOption::GRID)
			[[nodiscard]] auto operator()(
				options<Option>,
				const element::box_grid_builder::size_type width,
				const element::box_grid_builder::size_type height
			) const noexcept -> element::box_grid_builder
			{
			...
			template<element::BoxOption Option>
				requires (Option == element::BoxOption::GRID)
			[[nodiscard]] auto operator()(options<Option>, element::BoxGrid::element_matrix_type matrix) const noexcept -> element_type
			{
				return make_element<element::BoxGrid>(std::move(matrix));
			}
			...
			{
				return make_element<element::BoxGrid>(std::move(elements)...);
			}
			...
			// {
			// 	return element::box_grid_increasing_builder{}(std::move(elements)...);
			// }
			...
			[[nodiscard]] auto operator()(options<Option>) const noexcept -> auto
			{
				return functional::overloaded{
						[](const element::box_grid_builder::size_type width, const element::box_grid_builder::size_type height) noexcept -> element::box_grid_builder
						{
							return {.width = width, .height = height};
						},
						[]<typename... Args>(const element::box_grid_builder::size_type width, const element::box_grid_builder::size_type height, Args&&... args) noexcept -> auto
						{
							return element::box_grid_builder{.width = width, .height = height}(std::forward<Args>(args)...);
						},
						[](element::BoxGrid::element_matrix_type matrix) noexcept -> element_type
						{
							return make_element<element::BoxGrid>(std::move(matrix));
						},
						...
						// [](derived_element_t auto... elements) noexcept -> element::box_grid_increasing_builder
						// {
						// 	return element::box_grid_increasing_builder{}(std::move(elements)...);
						// },
				};
		...
		template<element::center_option_t auto... Os>
		struct maker<Os...>
		{
			template<element::CenterOption Option>
			[[nodiscard]] constexpr auto operator()(options<Option>) const noexcept -> auto
			{
				return [](element_type element) noexcept -> element_type
				{
					const auto filler = element::make_box_filler();

					if constexpr (constexpr auto option_value = std::to_underlying(Option);
						option_value == draw::element::center.horizontal)
					{
						return make(draw::element::box.horizontal)(filler, std::move(element), filler);
					}
					else if constexpr (option_value == draw::element::center.vertical)
					{
						return make(draw::element::box.vertical)(filler, std::move(element), filler);
					}
					else if constexpr (option_value == draw::element::center.all)
					{
						return make(draw::element::box.horizontal)(make(draw::element::box.vertical)(std::move(element)));
					}
					else
					{
						GAL_PROMETHEUS_SEMANTIC_STATIC_UNREACHABLE();
			...
			template<element::CenterOption Option, typename... Args>
			[[nodiscard]] auto operator()(options<Option> option, Args&&... args) const noexcept -> auto //
			{
				return this->operator()(option)(std::forward<Args>(args)...);
			}
