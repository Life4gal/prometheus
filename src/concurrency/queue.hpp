...
namespace gal::prometheus::concurrency
{
	...
	private:
			...
			requires requires { value_type{std::forward<Args>(args)...}; }
		{
			...
			impl_type::do_push_atomic(states_[index], elements_[index], value_type{std::forward<Args>(args)...});
		}
		...
		{
			const auto index = queue_detail::mapping_index<shuffle_bits>(head % element_count);
			impl_type::do_push_atomic(states_[index], elements_[index], static_cast<value_type>(std::forward<U>(value)));
		}
