...
GAL_PROMETHEUS_COMPILER_MODULE_NAMESPACE_INTERNAL(concurrency)
{
...
#endif
{
	...
	>
	class FixedAtomicQueue : public GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<FixedAtomicQueue<T, ElementCount, NilValue, MinimizeContention, MaximizeThroughput, IsTotalOrdered, IsSingleProducerSingleConsumer>>
	{
		using impl_type = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<FixedAtomicQueue>;
		friend impl_type;
	...
	public:
		...
		constexpr static auto elements_in_cache_line = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size / sizeof(atomic_value_type);
		constexpr static auto shuffle_bits = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_shuffle_bits<element_count, elements_in_cache_line>::value;

	private:
		alignas(GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size) atomic_value_type elements_[element_count];

		constexpr auto do_pop(const size_type tail) noexcept -> value_type
		{
			auto& atomic_element = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::mapping<shuffle_bits>(elements_, tail % element_count);
		...
		{
			auto& atomic_element = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::mapping<shuffle_bits>(elements_, head % element_count);
	...
	template<
		...
		bool IsSingleProducerSingleConsumer = false
	>
	class FixedQueue : public GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<FixedQueue<T, ElementCount, MinimizeContention, MaximizeThroughput, IsTotalOrdered, IsSingleProducerSingleConsumer>>
	{
		using impl_type = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<FixedQueue>;
		friend impl_type;
	...
	public:
		...
		constexpr static auto element_count = minimize_contention ? std::bit_ceil(ElementCount) : ElementCount;
		constexpr static auto elements_in_cache_line = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size / sizeof(atomic_state_type);
		constexpr static auto shuffle_bits = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_shuffle_bits<element_count, elements_in_cache_line>::value;

	private:
		alignas(GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size) atomic_state_type states_[element_count];
		alignas(GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size) value_type elements_[element_count];

		constexpr auto do_pop(const size_type tail) noexcept -> value_type
		{
			const auto index = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::mapping_index<shuffle_bits>(tail % element_count);
		...
		constexpr auto do_push(const size_type head, Args&&... args) noexcept -> void //
			requires requires { value_type{std::forward<Args>(args)...}; }
		{
			const auto index = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::mapping_index<shuffle_bits>(head % element_count);
	...
	template<
		...
		bool IsSingleProducerSingleConsumer = false
	>
	class DynamicAtomicQueue : public GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<DynamicAtomicQueue<T, NilValue, Allocator, MaximizeThroughput, IsTotalOrdered, IsSingleProducerSingleConsumer>>
	{
		using impl_type = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<DynamicAtomicQueue>;
		friend impl_type;
	...
	public:
		...
		constexpr static auto elements_in_cache_line = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size / sizeof(atomic_value_type);
		constexpr static auto shuffle_bits = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_shuffle_bits<0, elements_in_cache_line>::index_bits;
		constexpr static auto min_size = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::index_shuffle_bits<0, elements_in_cache_line>::min_size;

	private:
		alignas(GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::cache_line_size) size_type size_;
		...
		constexpr auto do_pop(const size_type tail) noexcept -> value_type
		{
			...
			auto& atomic_element = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::mapping<shuffle_bits>(elements_, index);
			return impl_type::template do_pop_atomic<value_type, nil_value>(atomic_element);
		}
	...
	template<
		...
		bool IsSingleProducerSingleConsumer = false
	>
	class DynamicQueue : public GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<DynamicQueue<T, Allocator, MaximizeThroughput, IsTotalOrdered, IsSingleProducerSingleConsumer>>
	{
		using impl_type = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::QueueImpl<DynamicQueue>;
		friend impl_type;
	...
	private:
		...
		constexpr auto do_pop(const size_type tail) noexcept -> value_type
		{
			const auto mask = size_ - 1;
			const auto index = tail & mask;
			const auto mapped_index = GAL_PROMETHEUS_COMPILER_MODULE_INTERNAL::mapping_index<shuffle_bits>(index);
