	...
	GAL_PROMETHEUS_COMPILER_NO_DESTROY unit_test::suite<"chars.utf16.le.scalar"> chars_utf16_le = []
	{
		...
		"to_latin"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				const auto output_length = Scalar<"utf16">::length<CharsType::LATIN, std::endian::little>(source);
				...
				using output_type = std::basic_string<char>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::LATIN, std::endian::little>(source);
				...
				}
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::LATIN, std::endian::little, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf8_char"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				expect(Scalar<"utf16">::validate<true, std::endian::little>(source) == "valid utf16 string"_b) << fatal;
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF8_CHAR, std::endian::little>(source);
				...
				using output_type = std::basic_string<char>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8_CHAR, std::endian::little>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8_CHAR, std::endian::little, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf8"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				expect(Scalar<"utf16">::validate<true, std::endian::little>(source) == "valid utf16 string"_b) << fatal;
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF8, std::endian::little>(source);
				...
				using output_type = std::basic_string<char8_t>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8, std::endian::little>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8, std::endian::little, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf16_le"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				expect(Scalar<"utf16">::validate<true, std::endian::little>(source) == "valid utf16 string"_b) << fatal;
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF16_LE, std::endian::little>(source);
				...
				using output_type = std::basic_string<char16_t>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF16_LE, std::endian::little>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF16_LE, std::endian::little, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf16_be"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				expect(Scalar<"utf16">::validate<true, std::endian::little>(source) == "valid utf16 string"_b) << fatal;
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF16_BE, std::endian::little>(source);
				...
				using output_type = std::basic_string<char16_t>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF16_BE, std::endian::little>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF16_BE, std::endian::little, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf32"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF32, std::endian::little>(source);
				...
				using output_type = std::basic_string<char32_t>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF32, std::endian::little>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF32, std::endian::little, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
	...
	GAL_PROMETHEUS_COMPILER_NO_DESTROY unit_test::suite<"chars.utf16.be.scalar"> chars_utf16_be = []
	{
		...
		"to_latin"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				const auto output_length = Scalar<"utf16">::length<CharsType::LATIN, std::endian::big>(source);
				...
				using output_type = std::basic_string<char>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::LATIN, std::endian::big>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::LATIN, std::endian::big, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf8_char"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				expect(Scalar<"utf16">::validate<true, std::endian::big>(source) == "valid utf16 string"_b) << fatal;
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF8_CHAR, std::endian::big>(source);
				...
				using output_type = std::basic_string<char>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8_CHAR, std::endian::big>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8_CHAR, std::endian::big, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf8"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				expect(Scalar<"utf16">::validate<true, std::endian::big>(source) == "valid utf16 string"_b) << fatal;
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF8, std::endian::big>(source);
				...
				using output_type = std::basic_string<char8_t>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8, std::endian::big>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF8, std::endian::big, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
		...
		"to_utf16_le"_test = []
		{
			...
			expect(Scalar<"utf16">::validate<true, std::endian::big>(source) == "valid utf16 string"_b) << fatal;
			const auto output_length = Scalar<"utf16">::length<CharsType::UTF16_LE, std::endian::big>(source);
			...
			using output_type = std::basic_string<char16_t>;
			{
				...
				const auto result = Scalar<"utf16">::convert<CharsType::UTF16_LE, std::endian::big>(source);
				expect(dest == ref(result)) << fatal;
			}
			{
				output_type dest{};
				...
				const auto result = Scalar<"utf16">::convert<CharsType::UTF16_LE, std::endian::big, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
				expect(dest == ref(result)) << fatal;
			}
		};
		...
		"to_utf16_be"_test = []
		{
			...
			expect(Scalar<"utf16">::validate<true, std::endian::big>(source) == "valid utf16 string"_b) << fatal;
			const auto output_length = Scalar<"utf16">::length<CharsType::UTF16_BE, std::endian::big>(source);
			...
			using output_type = std::basic_string<char16_t>;
			{
				...
				const auto result = Scalar<"utf16">::convert<CharsType::UTF16_BE, std::endian::big>(source);
				expect(dest == ref(result)) << fatal;
			}
			{
				output_type dest{};
				...
				const auto result = Scalar<"utf16">::convert<CharsType::UTF16_BE, std::endian::big, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
				expect(dest == ref(result)) << fatal;
			}
		};
		...
		"to_utf32"_test = []
		{
			for (std::size_t i = 0; i < trials; ++i)
			{
				...
				const auto output_length = Scalar<"utf16">::length<CharsType::UTF32, std::endian::big>(source);
				...
				using output_type = std::basic_string<char32_t>;
				{
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF32, std::endian::big>(source);
					expect(dest == ref(result)) << fatal;
				}
				{
					output_type dest{};
					...
					const auto result = Scalar<"utf16">::convert<CharsType::UTF32, std::endian::big, InputProcessPolicy::ASSUME_VALID_INPUT>(source);
					expect(dest == ref(result)) << fatal;
				}
			}
		};
