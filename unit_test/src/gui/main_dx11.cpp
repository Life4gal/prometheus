...
namespace
{
	...
	ComPtr<ID3D11ShaderResourceView> g_pic_texture;
	...
	auto d3d_create_device_objects() -> void
	{
		...
		// Create font texture
		{
			...
			g_draw_list_shared_data->get_default_font().texture_id = reinterpret_cast<gui::font_type::texture_id_type>(g_font_texture_view.Get());
	...
	[[nodiscard]] auto load_texture_from_file(const std::string_view filename) -> texture_type
	{
		texture_type result{.texture = nullptr, .width = 0, .height = 0};

		auto* data = stbi_load(filename.data(), &result.width, &result.height, nullptr, 4);
		if (data == nullptr)
		{
			return result;
		}
		...
		const D3D11_TEXTURE2D_DESC texture_2d_desc{
				.Width = static_cast<UINT>(result.width),
				.Height = static_cast<UINT>(result.height),
				.MipLevels = 1,
				.ArraySize = 1,
				.Format = DXGI_FORMAT_R8G8B8A8_UNORM,
				.SampleDesc = {.Count = 1, .Quality = 0},
		...
		{
				.pSysMem = data,
				.SysMemPitch = static_cast<UINT>(result.width * 4),
				.SysMemSlicePitch = 0
		};
		...
		check_hr_error(
			g_device->CreateShaderResourceView(texture.Get(), &shader_resource_view_desc, result.texture.GetAddressOf())
		);
	...
	auto prometheus_init() -> void //
	{
		...
		// load extra texture
		auto [texture, width, height] = load_texture_from_file(ASSETS_PATH_PIC);
		assert(texture != nullptr);

		g_pic_texture = texture;
	}
	...
	auto prometheus_render() -> void
	{
		...
		g_draw_list.image_rounded(reinterpret_cast<gui::DrawList::texture_id_type>(g_pic_texture.Get()), {900, 350, 1200, 650}, 10);
	}
	...
	auto prometheus_draw() -> void
	{
		...
		if (not g_vertex_buffer or vertex_list.size() > g_vertex_buffer_size)
		{
			// todo: grow factor
			g_vertex_buffer_size = vertex_list.size() + 5000;
		...
		if (not g_index_buffer or index_list.size() > g_index_buffer_size)
		{
			// todo: grow factor
			g_index_buffer_size = index_list.size() + 10000;
		...
		// g_device_immediate_context->DrawIndexed(static_cast<UINT>(index_list.size()), 0, 0);
