...
namespace
{
	...
	enum class MouseMod : std::uint8_t
	{
		NONE = 0,
	...
	enum class KeyboardMod : std::uint8_t
	{
		NONE = 0,
		SHIFT = GLFW_MOD_SHIFT,
		CONTROL = GLFW_MOD_CONTROL,
...
bool g_print_glfw_message = true;
bool g_print_glfw_message_focus = false;
bool g_print_glfw_message_cursor_enter = false;
bool g_print_glfw_message_cursor_position = false;
bool g_print_glfw_message_mouse_button = false;
bool g_print_glfw_message_scroll = false;
bool g_print_glfw_message_key = false;
bool g_print_glfw_message_char = false;
bool g_print_glfw_message_monitor = false;

auto glfw_callback_setup(GLFWwindow& w) -> void
{
	static auto callback_window_focus = [](GLFWwindow* window, const int focused) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_focus)
		{
	...
	static auto callback_window_cursor_enter = [](GLFWwindow* window, const int entered) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_cursor_enter)
		{
			std::println(
				stdout,
	...
	static auto callback_window_cursor_position = [](GLFWwindow* window, const double x, const double y) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_cursor_position)
		{
		...
		io.mouse_position = {static_cast<float>(x), static_cast<float>(y)};
	...
	static auto callback_window_mouse_button = [](GLFWwindow* window, const int button, const int action, const int mods) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_mouse_button)
		{
			std::println(
				stdout,
		...
		auto& io = gui::get_io();
		const auto pressed = static_cast<MouseAction>(action) == MouseAction::PRESS;

		switch (static_cast<MouseButton>(button))
		{
			case MouseButton::LEFT:
			{
				io.mouse_button_state[gui::MouseKey::LEFT] = pressed;

				break;
			}
			case MouseButton::MIDDLE:
			{
				io.mouse_button_state[gui::MouseKey::MIDDLE] = pressed;

				break;
			}
			case MouseButton::RIGHT:
			{
				io.mouse_button_state[gui::MouseKey::RIGHT] = pressed;

				break;
			}
			case MouseButton::X1:
			{
				io.mouse_button_state[gui::MouseKey::X1] = pressed;

				break;
			}
			case MouseButton::X2:
			{
				io.mouse_button_state[gui::MouseKey::X2] = pressed;
	...
	static auto callback_window_scroll = [](GLFWwindow* window, const double x, const double y) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_scroll)
		{
			std::println(stdout, "[MOUSE SCROLL]: window: 0x{:x}, x: {}, y: {}", reinterpret_cast<std::uintptr_t>(window), x, y);
		}

		auto& io = gui::get_io();
		io.mouse_wheel = static_cast<float>(y);
	...
	static auto callback_window_key = [](GLFWwindow* window, const int key_code, const int scan_code, const int action, const int mods) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_key)
		{
			std::println(
				stdout,
	...
	static auto callback_window_char = [](GLFWwindow* window, const unsigned int codepoint) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_char)
		{
	...
	static auto callback_window_monitor = [](GLFWmonitor* monitor, const int event) noexcept -> void
	{
		if (g_print_glfw_message and g_print_glfw_message_monitor)
		{
